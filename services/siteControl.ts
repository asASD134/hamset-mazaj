...
export type SiteControl = {
  ...
  /*
   * إظهار / إخفاء الصور داخل المعرض
   */
  gallery_images_visible: boolean[];

  /*
   * ⭐ اختيار الصور التي تظهر في الصفحة الرئيسية
   */
...
export async function updateSiteControl(
  payload: Partial<SiteControl>
): Promise<SiteControl> {
  ...
  if (
    ...
    const incoming =
      Array.isArray(
        payload.gallery_images_visible
      )
        ? payload.gallery_images_visible
        : [];
    ...
    const current =
      Array.isArray(
        existing.gallery_images_visible
      )
        ? existing.gallery_images_visible
        : [];
    ...
    const imageCount =
      Array.isArray(
        ...
        : Array.isArray(
          ...
          ? existing.gallery_images.length
          : 0;
   ...
   * =====================================================
   *
   * إذا لم تكن النجمة موجودة بعد لبعض الصور،
   * تكون القيمة false بشكل افتراضي للصور الجديدة.
   */

  let galleryImagesHome:
    | boolean[]
    | undefined;
   ...
   * عند إضافة صور جديدة:
   *
   * - نحافظ على إظهار/إخفاء الصور القديمة.
   * - الصور الجديدة تكون ظاهرة في المعرض.
   * - الصور الجديدة لا تظهر في الرئيسية
   *   إلا إذا اختارها المستخدم بالنجمة.
   * =====================================================
   */
  ...
  if (
    ...
    if (
      ...
      updatePayload.gallery_images_visible =
        images.map(
          (_, index) =>
            typeof currentVisibility[
              index
            ] === "boolean"
              ? currentVisibility[index]
              : true
        );
    }

    /*
     * المحافظة على النجوم القديمة
     */
    if (
      !Object.prototype.hasOwnProperty.call(
        payload,
      ...
      updatePayload.gallery_images_home =
        images.map(
          (_, index) =>
            typeof currentHome[
              index
