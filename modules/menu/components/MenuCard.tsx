...
export default function MenuCard({ item, design }: { item: MenuItem; design?: MenuDesign }) {
  ...
  const pricePositionClass = pricePosition[design?.pricePosition || "right"] || pricePosition.right;

  return (
    <article className={`group overflow-hidden border transition-all duration-500 hover:-translate-y-1 hover:border-yellow-500/40 ${radiusClass} ${cardBg} ${border} ${shadow}`}>
      {design?.showImages !== false && (
        <div className={`relative overflow-hidden ${ratioClass}`}>
          ...
          {design?.showPrices !== false && <div className={`absolute top-5 ${pricePositionClass} rounded-full px-5 py-2.5 text-xl font-black text-black shadow-lg whitespace-nowrap`} style={{ backgroundColor: accent }}>{item.price} ر.س</div>}
          {design?.showFeaturedBadge !== false && item.featured && (
      ...
      <div className="px-9 pb-8 pt-7">
        ...
        {design?.showPrices !== false && (
          <div className="mt-8 flex items-center justify-between gap-4 px-3">
            ...
            {hasTable ? (
              <button onClick={() => addToCart({ id: item.id, name: item.name, image: item.image, price: item.price, quantity: 1 })} className="flex items-center gap-3 rounded-2xl px-6 py-4 text-lg font-black text-black transition hover:scale-105 active:scale-95" style={{ backgroundColor: accent }}><ShoppingCart size={22} />إضافة للسلة</button>
            ) : (
              <div className="flex items-center gap-2 rounded-2xl border border-zinc-700 bg-zinc-800 px-6 py-4 text-zinc-300"><Eye size={20} />عرض فقط</div>
            )}
          </div>
        )}
      </div>
