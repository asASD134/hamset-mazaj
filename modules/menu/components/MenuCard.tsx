...
type MenuItem = { id: string; name: string; description: string; price: number; image: string };
...
export default function MenuCard({ item, design }: { item: MenuItem; design?: MenuDesign }) {
  ...
  return <article className={`group overflow-hidden border transition-all duration-500 hover:-translate-y-1 hover:border-yellow-500/40 ${radiusClass} ${cardBg} ${border} ${shadow}`}>
    {design?.showImages !== false && <div className={`relative overflow-hidden ${ratioClass}`}>
      ...
      {design?.showFeaturedBadge !== false && <div className="absolute left-5 top-5 flex items-center gap-1 rounded-full bg-black/70 px-3 py-2 text-yellow-400 backdrop-blur"><Star size={16} fill="currentColor" /><span className="text-sm font-bold">مميز</span></div>}
    </div>}
    <div className="p-7"><h3 className="mb-4 text-3xl font-black text-white">{item.name}</h3>
