...
const GLOBAL_SITE_KEYS = [
  "primary_color", "background_color", "surface_color", "typography", "section_order",
  ...
  "show_hero_badge", "show_hero_title", "show_hero_subtitle", "show_hero_description",
  "show_hero_primary_button", "show_hero_secondary_button", "show_featured_badge",
  "show_featured_title", "show_featured_description", "show_featured_products", "show_featured_prices", "show_featured_button",
...
export async function POST() {
  ...
  for (const cafe of cafes ?? []) {
    const { error } = await admin.from("site_control").update(update).eq("cafe_id", cafe.id);
    if (error) return NextResponse.json({ error: error.message, updatedCount }, { status: 500 });
    updatedCount += 1;
  }

  return NextResponse.json({ ok: true, updatedCount, version: new Date().toISOString() });
}
