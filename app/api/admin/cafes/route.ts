...
export async function POST(request: Request) {
  ...
  if (!name || !slug || !ownerEmail || !ownerPassword) {
    return NextResponse.json({ error: "اسم المقهى والرابط وبيانات المالك مطلوبة." }, { status: 400 });
  }
  if (!/^[a-z0-9-]{3,64}$/.test(slug)) {
    return NextResponse.json({ error: "الرابط يجب أن يحتوي على أحرف إنجليزية صغيرة وأرقام وشرطة فقط." }, { status: 400 });
  }
  ...
  if (userError || !createdUser.user) {
    return NextResponse.json({ error: userError?.message || "تعذر إنشاء حساب المالك." }, { status: 400 });
  }
  ...
  const { error: memberError } = await admin.from("cafe_members").insert({
    cafe_id: cafe.id,
  ...
  await admin.from("cafe_settings").insert({
    cafe_id: cafe.id,
    cafe_name: name,
    is_open: true,
  });

  await admin.from("site_control").insert({
    cafe_id: cafe.id,
    site_name: name,
  });
