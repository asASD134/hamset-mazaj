...
function getRequestedContext() {
  ...
  if (isAdminPath() && fromUrl) {
    writeStorage(fromUrl);
    return fromUrl;
  }

  if (isPlatformSettingsMode()) {
    writeStorage(PLATFORM_TEMPLATE_SLUG);
    return PLATFORM_TEMPLATE_SLUG;
  }
  ...
  return fromUrl === PLATFORM_TEMPLATE_SLUG
    ? DEFAULT_CAFE_SLUG
    : fromUrl || DEFAULT_CAFE_SLUG;
}
...
export function isPlatformSettingsClientMode() {
  return isPlatformSettingsMode();
}
