  ...
  useEffect(() => {
    ...
    fetch("/api/admin/platform-settings", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (active) setForm({ ...defaults, ...(result?.settings?.foundation || {}) });
      })
      .catch((error) => {
  ...
  return (
    <div dir="rtl" className="mb-6 rounded-3xl border border-yellow-500/20 bg-[#0b0d12] p-5 sm:p-7">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <label><span className="mb-2 block text-xs font-bold text-zinc-400">لون المنيو</span><div className="flex gap-2"><input type="color" value={String(form.menu_accent_color)} onChange={(e) => update("menu_accent_color", e.target.value)} className="h-12 w-16 rounded-xl border border-white/10 bg-zinc-900 p-1" /><input value={String(form.menu_accent_color)} onChange={(e) => update("menu_accent_color", e.target.value)} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-zinc-900 px-4 py-3 text-white" /></div></label>
      </div>

      <div className="mt-6 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        {([
          ["menu_show_images", "إظهار الصور"],
          ["menu_show_descriptions", "إظهار الوصف"],
          ["menu_show_prices", "إظهار الأسعار"],
          ["menu_show_featured_badge", "شارة المميز"],
